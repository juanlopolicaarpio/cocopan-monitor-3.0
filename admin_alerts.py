...
(Manila-time, cooldown persistence)
"""
import json
import logging
import smtplib
...
# --- Timezone utilities (ZoneInfo preferred, pytz fallback) ---
try:
...
def now_mnl() -> datetime:
    """Timezone-aware 'now' in Asia/Manila."""
    return datetime.now(MANILA_TZ)

def fmt_mnl(dt: Optional[datetime] = None) -> str:
    """Human-readable Manila timestamp for emails/UX."""
    if dt is None:
        dt = now_mnl()
    # Example: September 02, 2025 • 12:34 PM Manila Time
    return dt.strftime('%B %d, %Y • %I:%M %p Manila Time')


# --- Config import (kept as-is) ---
from config import config

# --- Logger setup with Manila timestamps ---
logging.basicConfig(level=getattr(logging, config.LOG_LEVEL))
logger = logging.getLogger(__name__)

class ManilaFormatter(logging.Formatter):
    def formatTime(self, record, datefmt=None):
        dt = datetime.fromtimestamp(record.created, MANILA_TZ)
        if datefmt:
            return dt.strftime(datefmt)
        return dt.isoformat()

_handler = logging.StreamHandler()
_handler.setFormatter(ManilaFormatter(
...
class AdminAlertManager:
    ...
    def __init__(self):
        ...
        self.last_alerts_file = Path(".last_alerts_mnl.json")
        self.config = self._load_config()
        self.last_alerts: Dict[str, datetime] = {}  # Manila-aware timestamps
    ...
    def _save_last_alerts(self) -> None:
        try:
            ...
            logger.error(f"Failed to persist last alerts: {e}")
        ...
        try:
            if self.last_alerts_file.exists():
                ...
                for k, iso in raw.items():
                    # Parse and normalize to Manila tz
                    try:
            ...
            logger.error(f"Failed to load last alerts: {e}")
    ...
    def _should_send_alert(self, alert_type: str) -> bool:
        ...
        if last_sent:
            ...
            if time_since < timedelta(minutes=cooldown_minutes):
                remaining = cooldown_minutes - (time_since.total_seconds() / 60.0)
                logger.debug(
                    f"Alert {alert_type} in cooldown for {remaining:.1f} more minutes"
                )
                return False
    ...
    def send_manual_verification_alert(self, problem_stores: List[ProblemStore]) -> bool:
        ...
        try:
            ...
            # Text version
            text_body = (
                f"🚨 CocoPan Monitor - Manual Verification Required\n"
                f"ALERT TIME: {current_time}\n"
                f"ISSUE: {len(problem_stores)} stores showing technical issues (not genuine closures)\n\n"
            )
...
<li style="margin-bottom: 10px;">
  ...
  <em style="color: #dc2626;">Issue: {store.message}</em>
            ...
            text_body += (
                ...
                f"2. Login to admin dashboard: {dashboard_url}\n"
...
"""
            ...
            if success:
                self.last_alerts['manual_verification_needed'] = now_mnl()
    ...
    def send_bot_detection_alert(self, blocked_count: int) -> bool:
        ...
        text_body = (
            "🤖 CocoPan Monitor - Bot Detection Alert\n"
            f"ALERT TIME: {current_time}\n"
            "ISSUE: High number of stores blocked by anti-bot measures\n"
            f"BLOCKED COUNT: {blocked_count} stores\n\n"
            ...
            f"Admin Dashboard: {self.config['email_settings']['dashboard_url']}\n"
        )
        ...
        if success:
            self.last_alerts['bot_detection_spike'] = now_mnl()
            self._save_last_alerts()
    ...
    def send_system_health_alert(self, cycle_time: float, database_errors: int) -> bool:
        ...
        issues = []
        max_cycle_sec = self.config['alert_triggers']['system_health']['max_cycle_time_minutes'] * 60
        if cycle_time > max_cycle_sec:
            issues.append(
                f"Monitoring cycle took {cycle_time/60:.1f} minutes "
                f"(expected < {self.config['alert_triggers']['system_health']['max_cycle_time_minutes']} min)"
            )
        ...
        text_body = (
            "⚠️ CocoPan Monitor - System Health Warning\n"
            f"ALERT TIME: {current_time}\n"
            "ISSUES DETECTED:\n"
        )
        for issue in issues:
        ...
        if success:
            self.last_alerts['system_health'] = now_mnl()
            self._save_last_alerts()
